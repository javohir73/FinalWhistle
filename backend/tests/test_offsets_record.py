...
def _seed_pair(db):
    ...
    for mv, probs, sh in (("poisson-elo-v0.2", (0.55, 0.25, 0.20), False),
                          (OFFSETS_MODEL_VERSION, (0.70, 0.18, 0.12), True)):
...
def test_fails_closed_without_token(monkeypatch):
    ...
    client, _ = _client()
    try:
        assert client.get("/api/internal/offsets-record").status_code == 503
    finally:
...
def test_rejects_bad_token(monkeypatch):
    ...
    client, _ = _client()
    try:
        assert client.get("/api/internal/offsets-record").status_code == 401
        assert client.get("/api/internal/offsets-record",
