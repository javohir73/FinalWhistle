...
export async function renderHomePage(competition: CompetitionId = "wc26") {
  const [teams, groups, matches, odds] = await Promise.all([
    getTeamsServer(competition).catch(() => null),
    getGroupsServer(competition).catch(() => null),
    getUpcomingMatchesServer(competition).catch(() => null),
